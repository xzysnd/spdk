...
static void
bdev_secondary_init_done(void *cb_arg, int rc)
{
	struct spdk_hot_upgrade_shared_state *state;

	if (rc != 0) {
		SPDK_ERRLOG("HU: bdev_initialize failed in secondary (rc=%d), "
			    "infrastructure may be partial\n", rc);
	}

	if (spdk_hot_upgrade_state_load(&state) == 0) {
		SPDK_NOTICELOG("HU: bdev_secondary_pre_init reconstructing %u bdevs\n",
			       state->num_bdev_infos);
		spdk_bdev_hu_reconstruct_bdevs(state->bdev_infos,
					       state->num_bdev_infos);
	} else {
		SPDK_ERRLOG("HU: bdev_secondary_pre_init failed to load state\n");
	}
...
{
	SPDK_NOTICELOG("HU: bdev_secondary_pre_init initializing bdev subsystem\n");
	spdk_bdev_initialize(bdev_secondary_init_done, NULL);
}
