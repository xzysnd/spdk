...
struct spdk_hu_bdev_info {
    ...
    char bdev_name[SPDK_HU_NAME_LEN];     /* Bdev name for RB tree rebuild (spdk_bdev_get_by_name) */
    uint32_t block_size;                  /* Block size for bdev reconstruction */
    uint32_t padding;                     /* Alignment padding */
    uint64_t num_blocks;                  /* Number of blocks for bdev reconstruction */
};

/**
 * vhost device info for cross-process device list rebuild during hot upgrade.
 * Primary saves device construction parameters; Secondary uses them to
 * rebuild g_vhost_devices TAILQ without replaying RPC.
 */
struct spdk_hu_vhost_dev_info {
    char     name[SPDK_HU_NAME_LEN];     /* Controller name, e.g. "naa.Malloc0.0" */
    char     dev_name[SPDK_HU_NAME_LEN]; /* Backend bdev name, e.g. "Malloc0" */
    char     path[SPDK_HU_PATH_LEN];     /* vhost socket path */
    uint32_t readonly;                    /* Read-only flag */
    uint32_t transport_type;             /* 0=blk, 1=scsi */
};
...
struct spdk_hot_upgrade_shared_state {
	...
	/* vhost subsystem device info for Secondary rebuild */
	uint32_t num_vhost_dev_infos;
	struct spdk_hu_vhost_dev_info vhost_dev_infos[SPDK_HU_MAX_VHOST_DEVS];
