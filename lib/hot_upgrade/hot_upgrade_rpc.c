...
struct rpc_secondary_init_ctx {
	struct spdk_jsonrpc_request *request;
};

static void
secondary_init_complete(void *arg)
{
	struct rpc_secondary_init_ctx *ctx = arg;
	struct spdk_json_write_ctx *w;
...
static void
secondary_init_after_takeover(void *arg)
{
	struct rpc_secondary_init_ctx *ctx = arg;

	spdk_hot_upgrade_timeline_record(SPDK_HU_TSC_SECONDARY_TAKEOVER_DONE, spdk_get_ticks());
	spdk_app_secondary_full_init(secondary_init_complete, ctx);
}

static void
secondary_init_after_pre_init(void *arg)
{
	struct rpc_secondary_init_ctx *ctx = arg;

	spdk_hot_upgrade_set_state(SPDK_HU_SECONDARY_TAKEOVER);
	spdk_subsystem_secondary_takeover(secondary_init_after_takeover, ctx);
}
...
rpc_secondary_init(struct spdk_jsonrpc_request *request,
		   const struct spdk_json_val *params)
{
	struct rpc_secondary_init_ctx *ctx;
	...
	/*
	 * pre_init is async: bdev subsystem calls spdk_bdev_initialize which
	 * takes a completion callback. When all subsystems complete pre_init,
	 * secondary_init_after_pre_init starts the takeover phase, which in
	 * turn calls full_init and sends the RPC response.
	 */
	spdk_subsystem_secondary_pre_init(secondary_init_after_pre_init, ctx);
}
SPDK_RPC_REGISTER("secondary_init", rpc_secondary_init, SPDK_RPC_RUNTIME)

/*
 * Functional-test RPC: force bdev reconstruction from saved state file.
 * Bypasses the hot upgrade state machine (which requires DPDK secondary
 * process mode). Used in Plan B functional testing where the primary was
 * killed to release the core lock, causing the new process to start as
 * a primary (IDLE state) rather than a DPDK secondary.
 */
static void
rpc_secondary_force_reconstruct(struct spdk_jsonrpc_request *request,
				const struct spdk_json_val *params)
{
	struct spdk_hot_upgrade_shared_state *state = NULL;
	struct spdk_json_write_ctx *w;
	int rc;

	rc = spdk_hot_upgrade_state_load(&state);
	if (rc != 0 || state == NULL) {
		SPDK_ERRLOG("HU: force_reconstruct failed to load state: %d\n", rc);
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
						 "Failed to load hot upgrade state");
		return;
	}

	SPDK_NOTICELOG("HU: force_reconstruct reconstructing %u bdevs\n",
		       state->num_bdev_infos);

	rc = spdk_bdev_hu_reconstruct_bdevs(state->bdev_infos,
					    state->num_bdev_infos);
	...
	spdk_json_write_object_begin(w);
	spdk_json_write_named_string(w, "status", rc == 0 ? "reconstructed" : "partial");
	spdk_json_write_named_uint32(w, "num_bdev_infos", state->num_bdev_infos);
	spdk_json_write_named_int32(w, "rc", rc);
	spdk_json_write_object_end(w);

	spdk_jsonrpc_end_result(request, w);
}
SPDK_RPC_REGISTER("secondary_force_reconstruct", rpc_secondary_force_reconstruct,
		  SPDK_RPC_RUNTIME)
